...
export default function MealPlanClient({
  ...
  const [selectedDayState, setSelectedDayState] = useState<DayOfWeek>(mealPlan.days[0]?.day || 'monday')
  const [expandedMeal, setExpandedMeal] = useState<string | null>(null)

  // Persist selected day to localStorage
  const setSelectedDay = useCallback((day: DayOfWeek) => {
  ...
  // On mount, restore selected day from localStorage
  useEffect(() => {
    if (typeof window !== 'undefined') {
      const saved = localStorage.getItem(SELECTED_DAY_STORAGE_KEY) as DayOfWeek | null
      const validDays = mealPlan.days.map(d => d.day)
      if (saved && validDays.includes(saved)) {
        setSelectedDayState(saved)
      }
    }
  ...
  const selectedDay = selectedDayState
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-36 md:pb-0">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="md:hidden fixed left-0 right-0 bg-white border-t border-gray-200 px-4 py-3 z-40 flex gap-3 shadow-lg" style={{ bottom: 'calc(5rem + env(safe-area-inset-bottom, 0px))' }}>
          <Link
            href={`/prep-view/${mealPlan.id}`}
            className="btn-primary bg-gradient-to-r from-primary-600 to-primary-500 flex-1 text-center text-sm py-2.5 flex items-center justify-center gap-2"
          >
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <Link
            href={`/grocery-list/${mealPlan.id}`}
            className="flex-1 text-center text-sm py-2.5 flex items-center justify-center gap-2 border-2 border-primary-600 text-primary-600 font-medium rounded-lg bg-white hover:bg-primary-50 transition-colors"
        ...
        <Link
          href={`/prep-view/${mealPlan.id}`}
          className="block mt-8 p-4 bg-gradient-to-r from-primary-50 to-green-50 border border-primary-200 rounded-xl hover:from-primary-100 hover:to-green-100 transition-colors group"
        >
